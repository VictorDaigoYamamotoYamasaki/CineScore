...
public class ReviewController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ReviewResponseDTO> buscar(@PathVariable String id) {
    ...
    public ResponseEntity<List<ReviewResponseDTO>> listarPorUsuario(@PathVariable String userId) {
    ...
    @PatchMapping("/{reviewId}/watched-date")
    public ResponseEntity<ReviewResponseDTO> atualizarDataAssistido(
            @PathVariable String reviewId,
            @AuthenticationPrincipal User currentUser,
            @RequestBody Map<String, String> body) {
        LocalDate watchedAt = LocalDate.parse(body.get("watchedAt"));
        return ResponseEntity.ok(
                reviewService.atualizarDataAssistido(reviewId, currentUser.getId(), watchedAt));
    }
}
