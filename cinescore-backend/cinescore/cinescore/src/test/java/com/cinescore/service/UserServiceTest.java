...
class UserServiceTest {
    ...
    void deveAtualizarPerfilComSucesso() {
        ...
        when(userRepository.findById(1L)).thenReturn(Optional.of(usuarioMock));
        when(userRepository.existsByEmailAndIdNot("pedro.novo@test.com", 1L)).thenReturn(false);
        when(userRepository.existsByNameIgnoreCaseAndIdNot("Pedro Novo", 1L)).thenReturn(false);
        when(userRepository.save(any())).thenReturn(usuarioMock);

        userService.atualizarPerfil(1L, dto);
    ...
    void deveLancarExcecaoQuandoNomeJaExisteAoAtualizar() {
        ...
        when(userRepository.findById(1L)).thenReturn(Optional.of(usuarioMock));
        when(userRepository.existsByEmailAndIdNot("pedro@test.com", 1L)).thenReturn(false);
        when(userRepository.existsByNameIgnoreCaseAndIdNot("Nome Existente", 1L)).thenReturn(true);

        assertThatThrownBy(() -> userService.atualizarPerfil(1L, dto))
                .isInstanceOf(DuplicateResourceException.class)
