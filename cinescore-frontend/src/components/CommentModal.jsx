...
export default function CommentModal({ review, onClose, onSummaryChange }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setComments(c.data || [])
      setReactions(s.data.reactions || { counts: {}, myReactions: [] })
  ...
  async function handleSend() {
    ...
    if (t.length > 500) {
      setErroComentario(`Comentário deve ter no máximo 500 caracteres (atual: ${t.length}).`)
      return
    }
    ...
    try {
      ...
      setErroComentario(e.response?.data?.message || 'Erro ao enviar comentário.')
  ...
  async function handleDelete(commentId) {
    ...
    try {
      ...
      setErroComentario(e.response?.data?.message || 'Erro ao excluir comentário.')
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Contador + erro de comentário */}
        {(text.length > 0 || erroComentario) && (
          <div style={{ paddingLeft:'1.2rem', paddingBottom:'0.2rem', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            {erroComentario
              ? <span style={{ fontSize:'0.7rem', color:'#ff6b6b' }}>{erroComentario}</span>
              : <span />
            }
            <span style={{ fontSize:'0.7rem', color: text.length > 450 ? '#ff6b6b' : 'var(--text-muted)' }}>
              {text.length}/500
            </span>
          </div>
        )}
        ...
        <div style={{ padding:'0.8rem 1.2rem', borderTop:'1px solid var(--border)', display:'flex', gap:'0.6rem' }}>
          <input
            ...
            value={text}
            onChange={e => { setText(e.target.value); setErroComentario('') }}
