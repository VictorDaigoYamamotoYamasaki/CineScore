...
import { profileService, movieService, followerService, recommendationService, sessionHelper } from '../services/api'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    profileService.perfilPorId(targetId)
      .then(async ({ data }) => {
        ...
        const ids = data.favorites.filter(f => f.movieId).map(f => f.movieId)
        if (ids.length > 0) {
          const results = await Promise.allSettled(ids.map(id => movieService.buscarPorId(id)))
          const map = {}
          results.forEach((res, i) => {
            if (res.status === 'fulfilled') {
              const m = res.value.data
              map[ids[i]] = { title: m.title, poster: m.poster, year: m.year }
            }
          })
          setMovieMap(map)
        }
  ...
  async function handleToggleFollow() {
    ...
    try {
      if (following) {
        await followerService.unfollow(targetId)
        setFollowing(false)
        setProfile(prev => ({ ...prev, followerCount: (prev.followerCount || 1) - 1 }))
      } else {
        await followerService.follow(targetId)
        setFollowing(true)
        ...
        <div className="container" style={{ paddingTop:'2rem', paddingBottom:'3rem' }}>
          ...
          <div style={{ marginBottom:'3rem' }}>
            <div className="feed-header" style={{ marginBottom:'1.2rem' }}>
              <span className="feed-title">❤️ Filmes Favoritos</span>
              {isOwn && (
                  ...
                  <div key={fav.position} style={{ position:'relative' }}>
                    {fav.movieId && info ? (
                      ...
                      <div style={{ position:'relative' }}>
                        ...
                        >
                          {info.poster
                            ? <img src={info.poster} alt={info.title} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} />
                        ...
                        {isOwn && (
                          <button
                            onClick={() => handleRemoveFavorite(fav.position)}
                            title="Remover"
                            style={{
                        ...
                        {isOwn && (
                          <button
                            onClick={() => setPicker(fav.position)}
                            title="Trocar filme"
                            style={{
          ...
          <div>
            <div className="feed-header" style={{ marginBottom:'1.2rem' }}>
              <span className="feed-title">⭐ Avaliações</span>
              <span style={{ fontSize:'0.78rem', color:'var(--text-muted)' }}>
                {profile?.reviews?.length || 0} avaliação{profile?.reviews?.length !== 1 ? 'ões' : ''}
              </span>
            </div>
